...
class OpenApiManagingGamesTests extends OpenApiTestsBase {
    ...
    void getGamesWithAllParams(final VertxTestContext testContext) {
        ...
        get(
                testContext,
                "/games?started=true&prefix=group24&numberOfPlayers=3",
                null,
                this::assertOkResponse
        );
    }
    ...
    void createGame(final VertxTestContext testContext) {
        service.setDelegate(new ServiceAdapter() {
            ...
            public Game createGame(String prefix, int numberOfPlayers) {
                Game game = new Game("group24_0", 10, new MonopolyService().getTiles());
    ...
    void joinGame(final VertxTestContext testContext) {
        service.setDelegate(new ServiceAdapter() {
            ...
            public void joinGame(String playerName, String gameId) {
                ...
                gameManager.addGame(new Game("group24", 2, new MonopolyService().getTiles()));
                gameManager.joinGame(new Player("Alice"), "group24_0");
            }
        });
        post(
                testContext,
                "/games/group24_0/players",
                null,
                new JsonObject()
